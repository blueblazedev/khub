...
run_track "$sb" enable >/dev/null 2>&1
run_track "$sb" task "TICKET-1" >/dev/null 2>&1
set_ok=0; [ -s "$tf" ] && [ "$(head -1 "$tf")" = "TICKET-1" ] && set_ok=1
...
if [ "$set_ok" -eq 1 ] && printf '%s' "$show" | grep -q 'TICKET-1' \
   && [ "$tagged" = "TICKET-1" ] && printf '%s' "$by" | grep -q 'TICKET-1' && printf '%s' "$by" | grep -q 'out 350'; then
...
run_track "$sb" task "TICKET-KEEP" >/dev/null 2>&1
run_track "$sb" disable >/dev/null 2>&1
if [ -s "$tf" ] && [ "$(head -1 "$tf")" = "TICKET-KEEP" ]; then
